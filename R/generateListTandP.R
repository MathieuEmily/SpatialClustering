generateListTandP <- function(data,w0=owin(c(0,250),c(0,250)),Homogeneous=TRUE,Z=NULL,intensity=NULL,myPPM=NULL,start.points=NULL){
	...
	if (!Homogeneous & is.null(Z)){
			break("Covariate is missing for the inhomogeneous case")
		}
	...
	for (k in 1:length(start.points)){
		...
		ptmp <- getProb(data=data,window=wtmp,w0=w0,Homogeneous=Homogeneous,Z=Z,intensity=intensity,myPPM=myPPM)
